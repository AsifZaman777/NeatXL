...
export default function Home() {
  ...
  const getCleanedData = (
    ...
    cleanedData = cleanedData.map((row) =>
      row.map((cell) =>
        typeof cell === "string" ? cleanText(cell) : String(cell)
      )
    );
    ...
    if (options.removeProfanity) {
      cleanedData = cleanedData.map((row) =>
        row.map((cell) =>
          typeof cell === "string" ? removeProfanityWords(cell) : cell
        )
      );
    }
    ...
    if (options.removeStopWords) {
      cleanedData = cleanedData.map((row) =>
        row.map((cell) =>
          typeof cell === "string" ? removeStopWords(cell) : cell
        )
      );
    }
    ...
    if (options.maskSensitive) {
      cleanedData = cleanedData.map((row) =>
        row.map((cell) =>
          typeof cell === "string" ? maskSensitiveData(cell) : cell
        )
      );
    }
    ...
    if (options.fillEmptyWith && options.fillEmptyWith.trim() !== "") {
      cleanedData = cleanedData.map((row) =>
        row.map((cell) =>
          cell === "" || cell == null ? options.fillEmptyWith : cell
        )
      );
    }
  ...
  const getStatistics = () => {
    ...
    const numericCols = cleanedCsvData.headers.map((_, colIndex) => {
      const values = cleanedCsvData.data
    ...
    <div className="w-full 2xl:max-w-6xl 2xl:mx-auto border-2 border-green-500 rounded-lg bg-gradient-to-br from-green-50 to-emerald-100 p-4">
      ...
      <div className="rounded-xl shadow-lg border-2 border-green-200 overflow-hidden backdrop-blur-sm bg-white/90">
        {!csvData ? (
          <div className="p-8">
            <CSVUploader
              onUpload={(data: CSVData, fileType?: "csv" | "xlsx") =>
                handleFileUpload(data, fileType)
              }
            />
          </div>
        ) : (
          <div className="divide-y divide-green-200">
            <div className="p-6 bg-gradient-to-r from-green-50 to-emerald-50">
              ...
              {/* Statistics */}
              {statistics && (
                <div className="mt-2 flex items-center gap-2 px-2 py-1.5 bg-green-100/80 backdrop-blur border border-green-200 rounded-md text-[11px] leading-none font-medium text-green-700">
                  <span className="flex items-center gap-1 font-semibold text-green-800">
                    <span>📊</span>
                    <span>Statistics</span>
                  </span>
                  <span className="flex items-center gap-1">
                    <span className="px-1.5 py-0.5 bg-white rounded border border-green-200">
                      Rows {statistics.totalRows}
                    </span>
                    <span className="px-1.5 py-0.5 bg-white rounded border border-green-200">
                      Cols {statistics.totalCols}
                    </span>
                  </span>
                  <button
                    type="button"
                    onClick={() =>
                      navigator.clipboard.writeText(
                        `${statistics.totalRows}x${statistics.totalCols}`
                      )
                    }
                    title="Copy dimensions (Rows x Cols)"
                    className="ml-1 inline-flex items-center h-6 px-2 rounded border border-green-300 bg-white text-green-700 hover:bg-green-50 active:scale-[0.96] transition"
                  >
                    📋
                  </button>
                  <div className="ml-auto flex items-center gap-1">
                    <button
                      ...
                      disabled={processing}
                      title="Export as CSV"
                      className={`h-7 inline-flex items-center gap-1 px-2 rounded border border-green-300 bg-green-800 text-white hover:bg-green-700 focus:outline-none focus:ring-1 focus:ring-green-400 disabled:opacity-50 disabled:cursor-not-allowed transition text-[11px] ${
                    ...
                    <button
                      ...
                      className={`h-7 inline-flex items-center gap-1 px-2 rounded border border-emerald-300 bg-emerald-600 text-white hover:bg-emerald-700 focus:outline-none focus:ring-1 focus:ring-emerald-400 disabled:opacity-50 disabled:cursor-not-allowed transition text-[11px] ${
                        processing && "pointer-events-none"
                      }`}
                    >
                      📊 Export as XLSX
                    ...
                    <button
                      ...
                      className={`h-7 inline-flex items-center gap-1 px-2 rounded border border-green-400 bg-gray-500 text-white hover:bg-green-800 focus:outline-none focus:ring-1 focus:ring-green-500 disabled:opacity-50 disabled:cursor-not-allowed transition text-[11px] ${
                        processing && "pointer-events-none"
                      }`}
                    >
                      📄 Export as JSON
