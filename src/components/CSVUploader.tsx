    ...
    Papa.parse(file, {
      ...
      complete: (results) => {
        ...
        const data = results.data.map((row) => 
          headers.map(header => (row as Record<string, string>)[header] || '')
        );
  ...
  return (
    ...
    `}>
      ...
      <div className="flex flex-col items-center justify-center gap-4">
        ...
        {isDragActive ? (
          <p className="text-lg font-medium text-blue-500">Drop the CSV file here</p>
        ) : (
          <div>
            <p className="text-lg font-medium text-gray-700">Drag & drop a CSV file</p>
